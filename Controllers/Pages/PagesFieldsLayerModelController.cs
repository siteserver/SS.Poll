...
namespace SS.Poll.Controllers.Pages
{
    ...
    public class PagesFieldsLayerModelController : ApiController
    {
        ...
        public IHttpActionResult Submit()
        {
            try
            {
                ...
                var fieldInfoDatabase =
                    FieldManager.GetFieldInfo(pollInfo.Id, fieldId) ??
                    new FieldInfo();

                string errorMessage;
                var isSuccess = fieldInfoDatabase.Id == 0 ? InsertFieldInfo(pollInfo.SiteId, pollInfo.Id, fieldInfo, isRapid, rapidValueList, out errorMessage) : UpdateFieldInfo(fieldInfoDatabase, fieldInfo, isRapid, rapidValueList, out errorMessage);
        ...
        private static bool InsertFieldInfo(int siteId, int pollId, FieldInfo body, bool isRapid, List<string> rapidValues, out string errorMessage)
        {
            ...
            if (FieldManager.Repository.IsTitleExists(pollId, body.Title))
            {
